...
{
	public class BackShopCpcReportController : ApiController
	{
		...
		/// <returns></returns>
		[Route("api/BackShopCpcReport/GetUserByShopCpcReport"), HttpGet]
		public HttpResponseMessage GetUserByShopCpcReport(int pageNum, int pagesize, string Statetime, string Endtime, int State, string KeyWord, int CountryId)
		{
			int count = Sum(Statetime, Endtime, State, KeyWord, CountryId);
			var list = UserByShopCpcReportList(pageNum, pagesize, Statetime, Endtime, State, KeyWord, CountryId);
		...
		{
			string where = "";
			string sql = "  select  count(k.Id)  from [dbo].[ShopCpcReport] AS k left join[dbo].[AroundUser] AS a on k.UserId=a.Id left join[dbo].[Country] as c on k.CountryId= c.Id where 1=1 ";
			if (State > 0)
			{
		...
		public static IList<UserByShopCpcReportModel> UserByShopCpcReportList(int pageNum, int pagesize, string Statetime, string Endtime, int State, string KeyWord, int CountryId)
		{
			int rownum = pagesize * (pageNum - 1);
			string where = "";
			string sql = " select k.*,a.Name,a.CustomerNumber,a.Phone,c.CountryName   from [dbo].[ShopCpcReport] AS k left join[dbo].[AroundUser] AS a on k.UserId=a.Id left join[dbo].[Country] as c on k.CountryId= c.Id where 1=1 ";
			if (State > 0)
			{
				where += " and k.State = " + State;
			}
			...
			using (SqlConnection conn = new SqlConnection(connstring))
			{
				...
				IList<UserByShopCpcReportModel> list = new List<UserByShopCpcReportModel>();
				while (dr.Read())
				{
					UserByShopCpcReportModel model = new UserByShopCpcReportModel();
					model.Id = (int)dr["Id"];
					model.UserId = (int)dr["UserId"];
					model.CountryId = (int)dr["CountryId"];
					model.ShopId = dr["ShopId"].ToString();
					model.State = (int)dr["State"];
					model.ShopCpcType = dr["ShopCpcType"].ToString();
		...
		/// <summary>
		/// time:2020-7-9
		/// message:店铺CPC体验报告改价格或改状态
		/// author:Thomars 
		/// </summary>
		/// <param name="model"></param>
		/// <returns></returns>
		[Route("api/BackShopCpcReport/ChangeUserByShopCpcReportStateOrMoney"), HttpPost]
		public IHttpActionResult ChangeUserByShopCpcReportStateOrMoney(ChangeBadEvaluateModel model)
		{
			int result = 0;
			if (model.State == 1)
			{
				...
				{
					int c = model.Id[i];
					var query = entities.ShopCpcReport.Where(e => e.Id == c).FirstOrDefault();
					if (query != null)
					{
						query.State = 2;
						...
						AroundUserFinanceLog log = new AroundUserFinanceLog
						{
							...
							Remarks = "任务：" + query.Id + " 店铺CPC体验报告支出"
		...
		/// <summary>
		/// time:2020-7-10
		/// message:店铺CPC体验报告Excel导入
		/// author:Thomars  
		/// </summary>
		/// <returns></returns>
		[Route("api/BackShopCpcReport/ExcelShopCpcReport"), HttpPost]
		public IHttpActionResult ExcelShopCpcReport()
		{
			int s = 0;
			int sum = 0;
			try
			{
				...
				var bad = new List<ExcelShopCpcReportModel>();
				if (filelist.Count > 0)
				{
					for (var i = 0; i < filelist.Count; i++)
					{
						...
						bad = dataTable.ToDataList<ExcelShopCpcReportModel>();//datatable转成list
					}
				}
				sum = bad.Count();
				using (TransactionScope scope = new TransactionScope())
				{
					foreach (var item in bad)
					{
						...
						var bid = entities.ShopCpcReport.Where(b => b.Id == item.编号).FirstOrDefault();
						if (item.状态.Equals("0"))
						{
							if (bid != null)
							{
		...
		/// <returns></returns>
		[Route("api/BackShopCpcReport/ShopCpcReportImages"), HttpPost]
		public IHttpActionResult ShopCpcReportImages([FromBody]JObject value)
		{
			int result = 0;
			int id = (int)value["Id"];
			string image = value["Image"].ToString();
			var query = entities.ShopCpcReport.Where(e => e.Id == id).FirstOrDefault();
			if (query != null)
			{
				query.ShopCpcType = image;
				DbEntityEntry entry = entities.Entry(query);
				entry.State = System.Data.Entity.EntityState.Modified;
				result = entities.SaveChanges();
			}
