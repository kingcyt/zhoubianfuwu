...
{
	public class BackShopReportController : ApiController
	{
		...
		/// <returns></returns>
		[Route("api/BackShopReport/GetUserByShopReport"), HttpGet]
		public HttpResponseMessage GetUserByShopReport(int pageNum, int pagesize, string Statetime, string Endtime, int State, string KeyWord, int CountryId)
		{
			int count = Sum(Statetime, Endtime, State, KeyWord, CountryId);
			var list = UserByShopReportList(pageNum, pagesize, Statetime, Endtime, State, KeyWord, CountryId);
		...
		{
			string where = "";
			string sql = "  select count(k.Id)  from ShopReport AS k left join[dbo].[AroundUser] AS a on k.UserId=a.Id left join[dbo].[Country] as c on k.CountryId= c.Id where 1=1 ";
			if (State > 0)
			{
		...
		public static IList<UserByShopReportModel> UserByShopReportList(int pageNum, int pagesize, string Statetime, string Endtime, int State, string KeyWord, int CountryId)
		{
			int rownum = pagesize * (pageNum - 1);
			string where = "";
			string sql = "  select k.*,a.Name,a.CustomerNumber,a.Phone,c.CountryName   from ShopReport AS k left join[dbo].[AroundUser] AS a on k.UserId=a.Id left join[dbo].[Country] as c on k.CountryId= c.Id where 1=1 ";
			if (State > 0)
			{
				where += " and k.State = " + State;
			}
			...
			using (SqlConnection conn = new SqlConnection(connstring))
			{
				...
				IList<UserByShopReportModel> list = new List<UserByShopReportModel>();
				while (dr.Read())
				{
					UserByShopReportModel model = new UserByShopReportModel();
					model.Id = (int)dr["Id"];
					model.UserId = (int)dr["UserId"];
					model.CountryId = (int)dr["CountryId"];
					model.ShopId = dr["ShopId"].ToString();
					model.State = (int)dr["State"];
					model.ShopType = dr["ShopType"].ToString();
					model.Price = dr["Price"].ToString();
					model.BuyTime = dr["BuyTime"].ToString();
					model.ShopEmail = dr["ShopEmail"].ToString();
		...
		/// <summary>
		/// time:2020-7-9
		/// message:店铺体验报告改价格或改状态
		/// author:Thomars 
		/// </summary>
		/// <param name="model"></param>
		/// <returns></returns>
		[Route("api/BackShopReport/ChangeUserByShopReportStateOrMoney"), HttpPost]
		public IHttpActionResult ChangeUserByShopReportStateOrMoney(ChangeBadEvaluateModel model)
		{
			int result = 0;
			if (model.State == 1)
			{
				...
				{
					int c = model.Id[i];
					var query = entities.ShopReport.Where(e => e.Id == c).FirstOrDefault();
					if (query != null)
					{
						query.State = 2;
		...
		/// <summary>
		/// time:2020-7-10
		/// message:店铺体验报告Excel导入
		/// author:Thomars 
		/// </summary>
		/// <returns></returns>
		[Route("api/BackShopReport/ExcelShopReport"), HttpPost]
		public IHttpActionResult ExcelShopReport()
		{
			int s = 0;
			int sum = 0;
			try
			{
				...
				var bad = new List<ExcelShopReportModel>();
				if (filelist.Count > 0)
				{
					for (var i = 0; i < filelist.Count; i++)
					{
						...
						bad = dataTable.ToDataList<ExcelShopReportModel>();//datatable转成list
					}
				}
				sum = bad.Count();
				using (TransactionScope scope = new TransactionScope())
				{
					foreach (var item in bad)
					{
						...
						var bid = entities.ShopReport.Where(b => b.Id == item.编号).FirstOrDefault();
						if (item.状态.Equals("0"))
						{
							if (bid != null)
							{
		...
		/// </summary>
		/// <returns></returns>
		[Route("api/BackShopReport/ShopReportImages"), HttpPost]
		public IHttpActionResult ShopReportImages([FromBody]JObject value)
		{
			int result = 0;
			int id = (int)value["Id"];
			string image = value["Image"].ToString();
			var query = entities.ShopReport.Where(e => e.Id == id).FirstOrDefault();
			if (query != null)
			{
				query.ShopType = image;
				DbEntityEntry entry = entities.Entry(query);
				entry.State = System.Data.Entity.EntityState.Modified;
				result = entities.SaveChanges();
			}
